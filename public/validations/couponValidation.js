...
const discountAmountErr = document.getElementById("discountAmountErr")
const maxDiscountAmountErr = document.getElementById("maxDiscountAmountErr")
...
function validateDiscount(){
    ...
    if(discount <= 0){
        discountAmountErr.innerHTML = 'Discount cannot be a negative number or zero'
        return false
    }else if(discount > 90){
        discountAmountErr.innerHTML = 'Upto 90% discount  only possible'
        return false
    }
...
function validateMaxDiscount(){
    ...
    if (maxDiscount <= 0) {
        maxDiscountAmountErr.innerHTML = 'Max Discount cannot be a negative number or zero';
        return false;
    } else if (maxDiscount > 90) {
        maxDiscountAmountErr.innerHTML = 'Upto 90% discount only possible';
        return false;
    }

    maxDiscountAmountErr.innerHTML = '';
    return true;
}
...
function validateCoupon(){
    return validateCode() && validateDiscount()&&validateMaxDiscount() && validateMinPurchase() && validateExpiryDate() && validateDescription()
}
